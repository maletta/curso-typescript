...
export class Votation {
  ...
  vote(votationIndex: number): Votation {
    // se esta opção de voto não existir
    if (!this._votationOptions[votationIndex]) return this;
...
votation1.vote(1).vote(1).vote(0).vote(0).vote(0).vote(2);
...
votation2.vote(1).vote(1).vote(1).vote(0).vote(0).vote(2);
